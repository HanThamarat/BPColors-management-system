   ...
   $objPHPExcel->getActiveSheet()->getStyle('A1:W1')->applyFromArray(
           array(
               ...
               'fill' => array(
                   ...
                   //for($i=1;$i<13;$i++){
                //     $claim = "SELECT * FROM tbl_claim WHERE payment_st='J วางบิลเรียบร้อย' AND total_pay='' AND insure_name!=' insure_name!=' order by insure_name";
                //    $q_claim = mysql_query($claim);
                    
                    $response = DB::table('tbl_claim')->whereRaw('payment_st = "" AND total_pay = "" AND  insure_name!="เคลมใน" order by insure_name')->get(); 
           ...
           return response()->download('example.xlsx');
    }
}
