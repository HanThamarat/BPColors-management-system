...
$objPHPExcel->getActiveSheet()->setCellValue('X1', 'ค่า Excess');
$objPHPExcel->getActiveSheet()->setCellValue('Y1', 'SA');
$objPHPExcel->getActiveSheet()->setCellValue('Z1', 'ยี่ห้อ');
$objPHPExcel->getActiveSheet()->setCellValue('AA1', 'รุ่นรถ');
$objPHPExcel->getActiveSheet()->setCellValue('AB1', 'EMCS');
$objPHPExcel->getActiveSheet()->setCellValue('AC1', 'วันที่DMS');
$objPHPExcel->getActiveSheet()->setCellValue('AD1', 'ประเภทงาน');
...
$objPHPExcel->getActiveSheet()->getStyle('A1:W1')->applyFromArray(
		array(
			...
			'fill' => array(
				...
				//  $claim = "SELECT * FROM tbl_claim WHERE (date_bill BETWEEN '".$fromdate."' AND '".$todate."') UNION
	            ...
	            // $q_claim = mysql_query($claim);
					...
					$objPHPExcel->getActiveSheet()->setCellValue('X'.($r+2),$res->exzept);
					$objPHPExcel->getActiveSheet()->setCellValue('Y'.($r+2),$res->user_con);
					$objPHPExcel->getActiveSheet()->setCellValue('Z'.($r+2),$res->car_brand);
					$objPHPExcel->getActiveSheet()->setCellValue('AA'.($r+2),$res->car_model);
					$objPHPExcel->getActiveSheet()->setCellValue('AB'.($r+2),$res->emcs);
					$objPHPExcel->getActiveSheet()->setCellValue('AC'.($r+2),$res->date_dms);
					$objPHPExcel->getActiveSheet()->setCellValue('AD'.($r+2),$res->car_job);
				$no++;	
				$r++;
				}
