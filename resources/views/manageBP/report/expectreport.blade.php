...
$objPHPExcel->getActiveSheet()->getStyle('A1:W1')->applyFromArray(
		array(
			...
			'fill' => array(
				...
				//for($i=1;$i<13;$i++){
                $response = DB::table('tbl_claim')->whereRaw("payment_st NOT IN ('K ชำระเงินแล้ว','L ยกเลิกงานเคลม') AND (date_bill is null OR bill_no='') AND total_pay=0  ORDER BY payment_st")->get();
...
header('Content-Disposition: attachment;filename="expectreport.xlsx"');
