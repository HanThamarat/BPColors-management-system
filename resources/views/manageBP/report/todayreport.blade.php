...
$objPHPExcel->getActiveSheet()->setCellValue('B1', 'วันที่ลงระบบ');
...
$objPHPExcel->getActiveSheet()->setCellValue('H1', 'ประกัน');
...
$objPHPExcel->getActiveSheet()->setCellValue('U1', 'ขออนุมัติค่าเเรง');
$objPHPExcel->getActiveSheet()->setCellValue('V1', 'ขออนุมัติค่าอะไหล่');
$objPHPExcel->getActiveSheet()->setCellValue('W1', 'อนุมัติค่าเเรง');
$objPHPExcel->getActiveSheet()->setCellValue('X1', 'อนุมัติค่าอะไหล่');
$objPHPExcel->getActiveSheet()->setCellValue('Y1', 'อนุมัติทั้งหมด');
$objPHPExcel->getActiveSheet()->setCellValue('Z1', 'ประเภทงาน');
$objPHPExcel->getActiveSheet()->setCellValue('AA1', 'ประเภทงาน');
$objPHPExcel->getActiveSheet()->setCellValue('AA1', 'ประเภทงาน');
...
$objPHPExcel->getActiveSheet()->getStyle('A1:W1')->applyFromArray(
		array(
			...
			'fill' => array(
                ...
                $response = DB::table('tbl_claim')->whereRaw("DATE_FORMAT(date_create,'%Y-%m-%d') BETWEEN ? AND ? ORDER BY date_status,payment_st DESC",["$fromdate", "$todate"])->get();
	              
                foreach ($response as $res) {
					$objPHPExcel->getActiveSheet()->setCellValue('A'.($r+2),$no);
					$objPHPExcel->getActiveSheet()->setCellValue('B'.($r+2),$res->date_create);
					...
					$objPHPExcel->getActiveSheet()->setCellValue('H'.($r+2),$res->insure_name);
					...
					$objPHPExcel->getActiveSheet()->setCellValue('U'.($r+2),$res->cost_doit);
					$objPHPExcel->getActiveSheet()->setCellValue('V'.($r+2),$res->cost_sparepart);
					$objPHPExcel->getActiveSheet()->setCellValue('W'.($r+2),$res->firm_doit);
					$objPHPExcel->getActiveSheet()->setCellValue('X'.($r+2),$res->firm_sparepart);
					$objPHPExcel->getActiveSheet()->setCellValue('Y'.($r+2),$res->firm_all);
					$objPHPExcel->getActiveSheet()->setCellValue('Z'.($r+2),$res->car_job);
...
header('Content-Disposition: attachment;filename="todayreport.xlsx"');
